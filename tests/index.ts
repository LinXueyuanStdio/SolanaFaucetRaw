...
async function main() {
  ...
  const faucetProgram = await deployer.ensure("faucetProgram", async () => {
    ...
    const soPath = "../build/libsolana_faucet.so"
  ...
  const faucetTokenOwner = await ProgramAccount.forSeeds([Buffer.from(testToken.publicKey.toBuffer()).slice(0, 30)], facuetPubkey)
  console.log("faucet token owner", {
    address: faucetTokenOwner.address,
    seed: faucetTokenOwner.seeds.toString(),
